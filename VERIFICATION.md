# 20 September 2026 V11.14 registered studies and accounting

**372 Python tests passed in 115.820 seconds on the final implementation.** Both actual-renderer JavaScript checks passed, including registered-study displays, separate realized/open marks, safe text rendering, disabled controls and account-reconciliation warnings. Python compilation, JavaScript syntax and diff whitespace checks passed. These are mocked-DOM checks; no hosted/mobile visual verification is claimed.

The real Gunicorn entry point passed local HTTP startup and restart with an isolated temporary database. Checks cover the dashboard/assets, private forward API authentication, invalid/backdated request rejection, an empty reconciled ledger, persisted settings and stopped runners. The first invocation lacked Gunicorn in the default Python environment; the successful run used the existing deployment dependency directory. No production dependency, financial limit or test timeout was changed to obtain a pass. See [the startup record](research_baselines/forward-web-startup.json).

New regressions cover exact break-even counts through closure and restart; same-second account commit ordering; independent fill and whole-account discrepancies; frozen/updating separation; normally closed labels versus open `END` marks; future-label rejection; unchanged earlier closed trades; missing, trailing, revised and late candles; sealed signal features; fixed registration costs/dates; export/restart equality; code-change records; and completion at the registered cutoff after an outage. Provider fixtures verify separate Massive bundle provenance, missing intervals, bounds, response validation, private header authentication, pacing and redirect rejection. These artificial fixtures establish software behavior, not investment returns.

The actual supplied XRP 15m replay uses **105,062 candles** and exactly reproduces the preceding V11.13 primary, higher-cost and frozen account metrics. Primary: **six trades, −$12.749657 net, $4.562520 fees**. Higher-cost primary: zero trades. Frozen diagnostic: three trades, +$8.329271. The model has 13,832 observations and remains unqualified. No rule was tuned or model promoted from that reused-history comparison. The new registered comparison has no collected future-performance evidence yet.

The replay and final `lab/*.py` source share SHA-256 `8b23909cb69f5e1565db5e511c583dacc768ac5dba3d5a0c4f770bcdcbb5f0d0`. Data SHA-256 is `1ea191a7f767c4e39f8a0801a563d091ebcf0861a053363e018c8a107067d236`. The 30.005-second replay runtime covers the primary workflow with optional experiments disabled; it is not a comparative speed measurement. See [the exact replay](research_baselines/forward-revision-replay.json) and [review record](research_baselines/forward-revision-review.json).

A fresh Coinbase candle request timed out. No new market candles were collected in this revision, and no Massive key was used for a live download. The downloader is tested with provider-shaped fixtures. Registered studies collect completed candles and use simulated fills, so their prospective design does not establish real quote execution. Existing observed-news collection remains included; the updating/frozen comparison does not isolate a news contribution. Engine V11.14/report 20 require fresh trading qualification; compatible v17 seeds can be studied as research. No hosted deployment, merge or exchange order was performed. Read [the design, sources and usage](FORWARD_STUDY.md).
...
New tests cover delayed first observations; future-content invariance; changed, cancelled and withdrawn schedules; Eastern DST; feed expiry and recovery; RSS/Atom parsing; asset matching and URL deduplication; revision persistence and atomic rejection; authenticated event export; exact report/event bundle identity; pinned inputs on study resume; entry-time forward/replay agreement; and independent event/price-control model training on explicitly artificial fixtures. The actual dashboard renderer also checks event headline/error escaping, rejected non-HTTPS links, date-only display and unavailable-source messaging. This renderer check is not a browser screenshot test.

A live adapter check collected **450 versions from six of seven configured sources**, including three upcoming entries within seven days. Coinbase status timed out and is reported unavailable. BLS and CoinDesk successfully loaded using the final adapter; an earlier attempt identified the canonical CoinDesk URL and differing source request behavior. Availability on Render still needs to be read from that deployment's source-status panel.

The real XRP 15m causality replay uses **105,062 supplied candles** and the untouched 450-version archive collected after the candle test period. It correctly reports **zero covered historical candles**, preserves all seven new event coefficients at zero, and reproduces V11.11's original model weights, calibration and forecast audit data (excluding only an earlier explanatory scope-text correction). The primary account still selects six trades for **−$12.749657**, pays $4.562520 in fees and remains unqualified; higher-cost primary selects zero trades. The frozen diagnostic account still selects three trades for +$8.329271. No strategy was selected or promoted from that diagnostic. This is not evidence that news improves profitability.

The replay's source hash matches the final `lab/*.py` source exactly: `29bc41ed5cb10ecb07d420de3e437c90a2b5f1c84bc0362fbce548448ab9e9c9`. Its 29.826-second measurement covers the primary workflow, not all optional experiments; no new before/after speed claim is made. See [the machine-readable record](research_baselines/market-events-replay.json) and [event sources, design and limitations](MARKET_EVENTS.md). Fresh policy v16/report 18 practice is required after deployment. Historical news coverage, surprise-release values and predictive benefit are not established by the existing uploads.
...
New causal regressions verify that blocked practice cannot make an eligible forecast ready or alter eligible weights/calibration; corrections cannot raise normal-policy entry estimates; failure estimates use only resolved labels and saved entry-time baselines; and unknown paths remain unknown. Bitcoin fixtures cover unfinished/future candles, gap resets, replay/forward parity, stale-context entry blocking, cache identity, state restoration and exact context exports. CLI routing, reviewed boundaries, interrupted/restarted trial records and regime-account reconciliation also pass. Artificial fixtures establish software behavior, not market returns.

The real Gunicorn entry point passed a local HTTP startup/restart smoke test with a fresh temporary database: dashboard/assets/health, private API authentication, invalid-plan and six-hour trading rejection, persisted settings and stopped runners. This does not establish hosted/mobile behavior or real fills. An end-to-end persistence fixture exceeded its old 120-second bound during concurrent research work; its test-only completion bound is now 180 seconds for the additional diagnostic work. It passes in final discovery. No production timeout, risk limit or financial qualification gate was loosened.

Three unchanged-baseline/revised pairs use identical supplied XRP 15m, DOT 15m and LTC 5m candles, completed daily inputs, fees, reviewed boundaries, development labels and training funnels. XRP's final primary account worsens from **-$4.976139 over four trades to -$12.749657 over six**. DOT and LTC select zero final trades in both versions. All primary higher-cost accounts also select zero final trades, and all six reports reject qualification. The fixed XRP exit experiment returns +$13.419072 at ordinary costs but -$7.471875 at higher costs and remains ineligible. No rule was tuned after reading these results. See [the research analysis](ELIGIBLE_CONTEXT_RESEARCH.md) and [exact comparison](research_baselines/eligible-context-comparison.json).

Failure heads improve on their entry-time historical-frequency baseline in only two of 18 comparisons, so they remain research diagnostics. No eligible LTC development candidate had enough prior evidence for a scored ready forecast. Keeping expensive blocked examples separate exposes this limitation without inventing evidence.

The supplied bundles lack Bitcoin daily candles and external retrieval was unavailable. The Bitcoin downloader, as-of join and forward behavior are implemented and fixture-tested, but their market-performance benefit remains unmeasured. Peak replay RSS is 416.5–451.6 MiB on these local inputs; a larger hosted plan remains unverified and should be sized using [the existing hosting guidance](WEBSITE_SETUP.md).

Final lab-source SHA-256: `7a8ed7e5d6ceb51048d9b15c982ef8a5eaddb72273fcae69cede17a193221b27`. The revised replay source is `506c5d8a1fbdc7403b5fb4b80734e8c6ea0fe96cace3898ed4552e049b110949`. Only one explanatory correction-scope string in `prediction_audit.py` changed afterward; reversing that string exactly reproduces the replay hash. All learning/execution calculations are identical. The final CLI and dashboard paths are included in the passing checks. The draft is not merged or deployed, and no exchange order was placed.
...
Both actual JavaScript mocked-DOM checks passed on the final UI. They exercise the latest supplied 11-market report, cost attribution, carried-in outcomes, saved multi-timeframe controls and submission validation, coverage dates/counts, research-only labels, separate finances for the same coin at different intervals, escaping and export handoff. Python compilation, JavaScript syntax and diff whitespace checks passed. No hosted/mobile visual verification, deployment or exchange orders were performed.

Seven continuity regressions cover pending outcomes across a review boundary, original entry forecasts, one-time delayed updates, future-label isolation, cooldown continuity, event-clock diagnostic counting, saved-model invariance and ordinary/higher-cost seed/control matching. Fourteen market-coverage regressions cover plan validation, limits, persistence, independent study identity, cross-timeframe reviewed history, protected forward profiles, job isolation/resume, history-length invalidation, retention of the selected lookback in automatic mode, unavailable listings, metadata failure reporting, coverage unions, cache retention and exact six-hour export/aggregation.

The identical-input LTC comparison reproduced the V11.9 handoff defect. At ordinary and higher costs, window feedback changed from 16 outcomes to 20, restored eight carried-in outcomes and matched the corresponding uninterrupted ending model exactly. Account net P&L remained $0.00. The uploaded model had 15,114 observations versus the correct continuous total of 15,118. The complete-bundle baseline/revised runs both mark the entire supplied dataset reviewed and remain unqualified. Their separate historical-window diagnostics do not qualify the strategy. See [the continuity analysis](CONFIRMATION_CONTINUITY_RESEARCH.md) and [exact comparison](research_baselines/confirmation-continuity-comparison.json).

Eight fixed higher-timeframe replays used the complete observed hourly and six-hour buckets from all four supplied coin datasets. They produced 17,747 model observations (14,939 development and 2,808 later practice), including overlapping candidates. Seven final accounts made no selected trade. XRP hourly lost $3.75 after ordinary costs and $4.59 at higher costs. All eight remained unqualified. The same underlying prices are reused, not newly independent evidence. See [coverage methods, sources and limits](MARKET_COVERAGE_RESEARCH.md) and [the eight-study record](research_baselines/timeframe-coverage.json).

The final lab-source SHA-256 is `3670f2704c3efcef824dbca17e77d6e3a85fe54bcc88a162e3cae20cdd775bc7`. Historical comparison records retain their exact earlier source hashes. The higher-timeframe replays predate only orchestration changes to retain selected lookback and restrict six-hour models to research; their directly called learning/execution modules, default costs and aggregation are unchanged. Direct Coinbase API access timed out in this development environment. New-coin and longer-history download paths are implemented and covered with fixtures, but no successful external expansion download is claimed. Existing uploaded hashes and all observed-bucket replays were verified locally.
