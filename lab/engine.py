...
ENGINE_VERSION="market-structure-v11.15-forecast-reliability"
...
def variants():
    """Base population across six classic families plus four sequence-first families."""
    out=[]
    ...
    # Sequence-first families. These wait for ordered market events and use retracement entries.
    for direction,fam in (
      ("LONG","sequence_reversal_long"),("SHORT","sequence_reversal_short"),
      ("LONG","sequence_continuation_long"),("SHORT","sequence_continuation_short")
    ):
        for stop,rr2,threshold in ((1.15,2.0,58),(1.4,2.4,62),(1.7,2.8,66)):
            out.append({"family":fam,"direction":direction,"threshold":threshold,
              "stop_atr":stop,"rr1":1.0,"rr2":rr2,"min_edge_probability":.48,
              "volume_z_min":-.5,"adx_min":0,"max_gap_atr":.28,
              "max_cost_r":.70,"time_stop_hours":16,"confirmation":2,
...
def build_feature_cache(rows,interval,simple_only=False,daily_rows=None):
    ...
    for i in range(240,n):
        ...
        F[i]={
          ...
          "_atr":a,"_close":c[i],"_ts":rows[i]["ts"],"_trend_long":e8[i]>e20[i]>e50[i] and e50[i]>e200[i],
        ...
        if simple_only:
            F[i]={key:F[i][key] for key in simple_keys}
    if simple_only:
...
def evaluate_signal(f,p,edge_prob=None,edge_lower=None,edge_samples=0):
    ...
    elif fam=="sequence_reversal_long":
        if not f.get("bullish_reversal_sequence"):
            return None,"no_bullish_reversal_sequence"
        score += 34
        score += 10 if f.get("liquidity_sweep_low") else 0
        score += 10 if f.get("choch_up") else (7 if f.get("bos_up") else 0)
        score += 8 if f.get("bull_retrace") else 0
        score += 6 if f.get("discount") else 0
        score += 6 if f.get("bull_fvg_active") else 0
        score += 5 if f.get("rejection_strength",0)>.12 else 0
        score += 4 if f.get("signed_volume_pressure",0)>=0 else -2

    elif fam=="sequence_reversal_short":
        if not f.get("bearish_reversal_sequence"):
            return None,"no_bearish_reversal_sequence"
        score += 34
        score += 10 if f.get("liquidity_sweep_high") else 0
        score += 10 if f.get("choch_down") else (7 if f.get("bos_down") else 0)
        score += 8 if f.get("bear_retrace") else 0
        score += 6 if f.get("premium") else 0
        score += 6 if f.get("bear_fvg_active") else 0
        score += 5 if f.get("bear_rejection_strength",0)>.12 else 0
        score += 4 if f.get("signed_volume_pressure",0)<=0 else -2

    elif fam=="sequence_continuation_long":
        if not f.get("bullish_continuation_sequence"):
            return None,"no_bullish_continuation_sequence"
        score += 32
        score += 10 if f.get("structure")=="BULL" else 0
        score += 9 if f.get("bos_up") or f.get("bars_since_bos_up",999)<=6 else 0
        score += 8 if f.get("bull_retrace") else 0
        score += 6 if f.get("bull_fvg_active") else 0
        score += 5 if f.get("mtf_alignment",0)>=.5 else 0
        score += 4 if f.get("signed_volume_pressure",0)>=-.02 else -2

    elif fam=="sequence_continuation_short":
        if not f.get("bearish_continuation_sequence"):
            return None,"no_bearish_continuation_sequence"
        score += 32
        score += 10 if f.get("structure")=="BEAR" else 0
        score += 9 if f.get("bos_down") or f.get("bars_since_bos_down",999)<=6 else 0
        score += 8 if f.get("bear_retrace") else 0
        score += 6 if f.get("bear_fvg_active") else 0
        score += 5 if f.get("mtf_alignment",0)<=-.5 else 0
        score += 4 if f.get("signed_volume_pressure",0)<=.02 else -2
...
def simulate(rows,features,start,end,balance,risk,fee_rate,base_slip,params,edge_model=None,keep_trades=True):
    from .execution import simulate as causal_simulate
    return causal_simulate(rows,features,start,end,balance,risk,fee_rate,base_slip,params,edge_model,keep_trades)
...
def robustness(m):
    ...
    dd=float(m["max_drawdown_pct"] if m["max_drawdown_pct"] is not None else 100)
...
def setup_intelligence_report(trades):
    ...
    concepts=[
      ...
      "bullish_reversal_sequence","bearish_reversal_sequence",
      "bullish_continuation_sequence","bearish_continuation_sequence"
    ]
    out={}
    for c in concepts:
...
def finalize_result(db_path,config,state):
    ...
    cf_samples=[]
    for f in folds:
        source=f.get("test") or {}
        if not (source.get("trades") or 0):
            source=f.get("validation") or f.get("train") or {}
        cf_samples.extend((source.get("signal_funnel") or {}).get("counterfactual_samples",[]))
    ...
    result={"symbol":config["symbol"],"interval":config["interval"],"rows":state["rows"],
            ...
            "capabilities":{"long_and_short":True,
                            "strategy_families":["trend_breakout","trend_pullback","liquidity_reclaim",
                                                 ...
                                                 "sequence_reversal_long","sequence_reversal_short",
                                                 "sequence_continuation_long","sequence_continuation_short"],
                            "features":["EMA trend structure","Donchian 20/55 breakout","RSI","ATR","ADX proxy",
                                        ...
                                        "liquidity sweep depth","daily bias score","ordered sweep→structure-shift→retrace sequences","retrace-limit entries"],
