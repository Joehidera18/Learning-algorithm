...
def _swing_flags(rows, span=3):
    ...
    A pivot at index p is not known until `span` later candles have closed.
    We therefore mark the *confirmation bar* i, not the historical pivot bar p.
    This prevents future leakage in walk-forward/backtest features.
    """
    n=len(rows)
    highs=[False]*n; lows=[False]*n
    for i in range(span*2, n):
        p=i-span
        window=rows[p-span:p+span+1]  # all bars are <= current confirmation bar i
        hi=rows[p]["high"]; lo=rows[p]["low"]
...
def build_structure_features(rows, atr_values):
    ...
    for i,r in enumerate(rows):
        ...
        if swing_hi[i]:
            p=max(0,i-3)
            recent_swing_highs.append((p,rows[p]["high"]))
        ...
        prior2=rows[i-2] if i>1 else prior
        bull_pullback_confirm=bool(
            r["close"]>r["open"] and
            (r["low"]<prior["low"] or prior["close"]<prior["open"] or prior2["close"]<prior2["open"])
        )
        bear_pullback_confirm=bool(
            r["close"]<r["open"] and
            (r["high"]>prior["high"] or prior["close"]>prior["open"] or prior2["close"]>prior2["open"])
        )
