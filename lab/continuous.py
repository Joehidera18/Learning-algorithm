...
DEFAULTS = {
    ...
    "max_total_risk": .03, "max_related_risk": .015, "max_notional_fraction": .30, "max_gross_exposure": 1.0,
...
BOUNDS = {
    ...
    "max_total_risk": (.005, .08), "max_related_risk": (.005, .08), "max_notional_fraction": (.05, 1),
...
class ContinuousLearner:
    ...
    def configure(self, patch):
        ...
        unknown = set(patch) - set(BOUNDS) - {"allow_shorts", "entries_paused", "validated_only", "decision_interval", "learning_enabled"}
        if unknown:
            ...
            try:
                with con:
                    ...
                    if not keep_memory:
                        ...
                        con.execute("DELETE FROM continuous_state WHERE key LIKE 'adaptive_paper_%'")
    ...
    def _candidate_opportunities(self, pid, f, ctx):
        if self.settings.get("learning_enabled"):
            from .adaptive import current_policy
            from .research import cost_signature
            policy = current_policy(self.db_path,pid,self.settings,"paper")
    ...
    def _decision_cycle(self, pid):
        ...
        with self.lock:
            ...
            if not opportunities:
                ...
                return
            exploration = (not self.settings.get("learning_enabled") and not self.settings["validated_only"]
                           and random.random() < self.settings["exploration_rate"])
            choice = random.choice(opportunities[:8]) if exploration else opportunities[0]
            if not self.settings.get("learning_enabled") and not self.settings["validated_only"] and not exploration and choice["learned"]["samples"] >= 25 and choice["learned"]["expectancy_r"] < -.10:
                m["last_decision"] = "Waiting: this setup has negative learned expectancy"
                return
            if not self.settings.get("learning_enabled") and not self.settings["validated_only"] and choice["adjusted_score"] < 54 and not exploration:
    ...
    def _entry_block(self, pid):
        ...
        if self.settings.get("learning_enabled") and not self._adaptive_profile(pid):
            return "Learning: no current qualifying model for this market and fee setting"
        if not self.settings.get("learning_enabled") and self.settings["validated_only"] and self._approved_params(pid) is None:
        ...
        if any(self._fresh_quote(held) is None for held in self.open_positions):
            return "Waiting: an open position has a stale quote; portfolio risk cannot be sized reliably"
    ...
    def _adaptive_profile(self, pid):
        from .adaptive import approved_profile
        return approved_profile(self.db_path,pid,self.settings)
    ...
    def _open_position(self, pid, market_price, f, ctx, choice, mode):
        with self.lock:
            ...
            qualified = self._adaptive_profile(pid) if self.settings.get("learning_enabled") else self._approved_params(pid)
            risk_scale = 1.0 if qualified else .25
            from .portfolio_risk import related_risk
            concentration = related_risk(pid, self.market, self.open_positions, now_ms(), direction)
            risk_budget = min(equity * self.settings["risk_per_trade"] * risk_scale,
                              equity * self.settings["max_related_risk"] - concentration["related_open_risk_usd"],
            ...
            if not math.isfinite(qty) or qty <= 0 or qty * entry < 1:
                self.market[pid]["last_decision"] = "Portfolio, related-market risk or available exposure is fully used"
                return None
            opened_at = now_ms()
            ...
            entry_decision["portfolio_concentration"] = dict(concentration,
                risk_limit_usd=equity*self.settings["max_related_risk"], scope="Paper account entry-time sizing")
            if self.events:
                ...
                with con:
                    ...
                    try:
                        ...
                        write_in_transaction(con,"adaptive_error_paper_"+pid,
                            {"message":str(exc),"trade_id":pos["trade_id"],"time":time.time()})
    ...
    def coinbase_signal(self, pid):
        """Export a fresh signal; paper outcomes never train the Coinbase model."""
        with self.lock:
            if not self.runtime["running"] or self.stop_event.is_set():
            ...
            if self.settings.get("learning_enabled"):
                from .adaptive import current_policy
                from .research import cost_signature
                policy = current_policy(self.db_path,pid,self.settings,"coinbase")
    ...
    def status(self):
        with self.lock:
            ...
            active_profiles = ({pid:{"kind":"adaptive"} for pid in self.product_ids if self._adaptive_profile(pid)}
                if self.settings.get("learning_enabled") else
                {pid: profile for pid, profile in stored_profiles.items() if self._approved_params(pid) is not None})
            return {"version": "11.0", "paper_only": True, "runtime": runtime, "portfolio": p,
