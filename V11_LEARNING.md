...
3. Collect hypothetical resolved examples for 16 candidate rules across four families, using only the development portion of the history. Explore signal-matched setups blocked by costs or net reward, still charging full costs. Invalid prices, entry gaps, signal requirements and sizing remain enforced. Discard unknown outcomes interrupted by missing candles.
...
Completed candidate labels and diagnostics are checkpointed in the database. A restart within 24 hours reuses a pinned candle cutoff and skips finished candidates if data, settings and versions match. The interrupted candidate and later evaluations restart. Older jobs or changed inputs discard incompatible training checkpoints; successful jobs remove them. Downloaded chunks remain reusable. The policy is `online-net-r-v3-historical-exploration`; report version 5 adds gap-aware history and costed training exploration. Old policy models require retraining; old reports remain viewable but are labeled for fresh practice and cannot supply a current qualifying model.
