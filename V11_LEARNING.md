...
The original 13 inputs include a constant, RSI, quote-volume z-score, ADX, ATR percentage, 20- and 50-bar momentum, a quote-volume OBV proxy, candle-volume pressure, close location within the candle, two regime flags, and candle-range expansion. Three additional inputs describe seven-day momentum, distance from the 20-day SMA in daily ATR units, and daily ATR percentage. Daily inputs use only complete UTC days as of the signal close. All inputs were available at the entry signal close. Model forecasts are estimates, not calibrated win probabilities or promises of profit.
...
Completed candidate labels and diagnostics are checkpointed in the database. A restart within 24 hours reuses a pinned candle cutoff and skips finished candidates if data, settings and versions match. The interrupted candidate and later evaluations restart. Older jobs or changed inputs discard incompatible training checkpoints; successful jobs remove them. Downloaded chunks remain reusable. The policy is `online-net-r-v4-daily-context`; report version 6 adds daily context, six swing candidates, and a comparison against the original 16 candidates. Gap-aware history and costed training exploration remain in place. See [STRATEGY_RESEARCH.md](STRATEGY_RESEARCH.md) for the fixed rules and source limitations. Old policy models require retraining; old reports remain viewable but are labeled for fresh practice and cannot supply a current qualifying model.
